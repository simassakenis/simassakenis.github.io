...
Suppose we have a vocabulary consisting of $$ V $$ distinct types (e.g., words of a language, letters of an alphabet, intensities of pixels). Let $$ \ldots, X_{-2}, X_{-1}, X_0, X_1, X_2, \ldots $$ be a seqeunce of random variables representing a sequence of tokens, where $$ X_j \in \{ 1, 2, \ldots, V \} $$ is the vocabulary ID of the $$ j $$-th token. If $$ J = (j_1, j_2, \ldots, j_k) $$ is a sequence of indices, let $$ \bm{X}_J $$ denote $$ (X_{j_1}, X_{j_2}, \ldots, X_{j_k}) $$. Also, let $$ \bm{X}_{i:j} $$ denote $$ (X_i, X_{i+1}, X_{i+2} \ldots, X_j) $$ for any integers $$ i,j $$, with $$ \bm{X}_{i:j} $$ an empty sequence if $$ i > j $$.

A language model is a statistical model of the conditional distribution of $$ \bm{X}_{1:n} \mid \bm{X}_{-\infty:0} $$, i.e., the probability distribution of a sequence of $$ n $$ tokens given a (theoretically infinite) history of tokens preceding it.

<!-- An autoregressive language model is a language model based on the following factorization of the probability of a sequence:

$$ P_\text{AR}(\bm{X}_{1:n} \mid \bm{X}_{-\infty:0}; \bm{\theta}) = \prod_{i=1}^n P_\text{AR}(\bm{X}_{1:i} \mid \bm{X}_{1:i-1}, \bm{X}_{-\infty:0}; \bm{\theta}). $$ -->

* The **GPT** model assumes that $$ \bm{X}_{1:n} $$ is independent of $$ \bm{X}_{-\infty:0} $$:

$$ P_\text{GPT}(\bm{X}_{1:n} \mid \bm{X}_{-\infty:0}; \bm{\theta}) = \prod_{i=1}^n P_\text{GPT}(\bm{X}_i \mid \bm{X}_{1:(i-1)}; \bm{\theta}). $$

* The **Transformer-XL** model assumes that $$ X_j $$ is independent of $$ \bm{X}_{-\infty:(j-m-1)} $$:

$$ P_\text{TXL}(\bm{X}_{1:n} \mid \bm{X}_{-\infty:0}; \bm{\theta}) = \prod_{i=1}^n P_\text{TXL}(\bm{X}_i \mid \bm{X}_{(i-m):(i-1)}; \bm{\theta}). $$

* The **XLNet** model assumes that $$ \bm{X}_{1:n} $$ is independent of $$ \bm{X}_{-\infty:-m} $$:

$$ P_\text{XLNet}(\bm{X}_{1:n} \mid \bm{X}_{-\infty:0}; \bm{\theta}) = \prod_{i=1}^n P_\text{XLNet}(\bm{X}_{z_i} \mid \bm{X}_{z_1:z_{i-1}}, \bm{X}_{(-m+1):0}; \bm{\theta}). $$

A masked language model is a statistical model of the conditional distribution of $$ \bm{X}_J \mid (\bm{X}_{1:n}', \bm{X}_{-\infty:0}) $$, where $$ \bm{X}_{1:n}' $$ is a corrupted sequence and $$ J $$ is a sequence of indices of the corrupted tokens. Specifically, $$ X_j' = I \xi + (1 - I) X_j $$ where $$ I \sim \text{Bern}(p) $$ for some probability $$ p \in [0, 1] $$ of corruption and $$ \xi \in \{ 1, 2, \ldots, V \} $$ is the index of the corruption token.
...
&nbsp;
## Statistical interpretation 3


Suppose we have a vocabulary consisting of $$ V $$ distinct types (e.g., words of a language, letters of an alphabet, intensities of pixels). Let $$ \bm{X} = (X_1, X_2, \ldots, X_T) $$ be a seqeunce of random variables representing a sequence of tokens, where $$ X_j \in \{ 1, 2, \ldots, V \} $$ is the vocabulary ID of the $$ j $$-th token. If $$ J = (j_1, j_2, \ldots, j_k) $$ is a sequence of indices, let $$ \bm{X}_J $$ denote $$ (X_{j_1}, X_{j_2}, \ldots, X_{j_k}) $$. Also, let $$ \bm{X}_{i:j} $$ denote $$ (X_i, X_{i+1}, X_{i+2} \ldots, X_j) $$ for any integers $$ i,j $$, with $$ \bm{X}_{i:j} $$ an empty sequence if $$ i > j $$.

A language model is a statistical model of distribution of $$ \bm{X} $$.

* GPT:

$$ P_\text{GPT}(\bm{X}; \bm{\theta}) = \prod_{i=1}^{T / n} P_\text{GPT}(\bm{X}_{in:(i+1)n}; \bm{\theta}). $$

* Transformer-XL:

$$ P_\text{TXL}(\bm{X}; \bm{\theta}) = \prod_{i=1}^{T / n} P_\text{TXL}(\bm{X}_{in:(i+1)n} \mid \bm{X}_{(in-m):(in-1)}; \bm{\theta}). $$
