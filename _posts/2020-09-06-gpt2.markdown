...
\begin{array}{@{}lll}
     ...
     \text{Embedding} & \bm{X}'^{(0)} = \text{OneHot}(\bm{x}) \bm{W}_e + \bm{W}_p &\hspace{10pt} (n \times H) \\
     &\bm{X}^{(0)} = \bm{X}'^{(0)} + \text{diag}(\bm{t}) \text{Stack}_n(\bm{w}_A) + \text{diag}(1 - \bm{t}) \text{Stack}_n(\bm{w}_B) &\hspace{10pt} (n \times H) \\
     \text{Layers} & \bm{X}'^{(l)} = \text{LayerNorm}(\text{MultiHeadSA}(\bm{X}^{(l-1)}, \text{Mask}^\text{BD}) + \bm{X}^{(l-1)}), &\hspace{10pt} (n \times H) \\
     & \bm{X}^{(l)} = \text{LayerNorm}(\text{FFN}_\text{RELU}(\bm{X}'^{(l)}) + \bm{X}'^{(l)}) \quad \text{for } l = 1, \ldots, L&\hspace{10pt} (n \times H) \\
     \text{Output} & \text{GPT}(\bm{x}) = \text{Softmax}\left( \bm{X}_{j_\text{masked},:}^{(L)} \bm{W}_e^\top \right) &\hspace{10pt} (V)
\end{array}
$$
</div>
